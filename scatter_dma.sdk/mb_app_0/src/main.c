...
/* Keep in mind, the linker was modified to include the .dmamem_section section.
This is necessary to ensure these arrays are stored in the external memory, not
the memory space of the application which is located on block ram. The dma cannot
access the block ram, so it is important the arrays are located in external memory.
It is also important that the application is stored in block ram so that the bitstream
can be compiled to also contain the program. */
volatile u16 __attribute__ ((aligned(0x100),section(".dmamem_section"))) tx_buff [ BUFF_TOTAL ];
volatile u32 __attribute__ ((aligned(0x100),section(".dmamem_section"))) rx_buff [ BUFF_TOTAL ];
volatile s16 __attribute__ ((aligned(0x100),section(".dmamem_section"))) sin_buff[ BUFF_TOTAL ];
